...
const app = {
    ...
    initGoogleMap(lat, lng) {
        ...
        this.googleMap = new google.maps.Map(mapDiv, {
            center: centerPos, zoom: 16, disableDefaultUI: true, zoomControl: true
        });

        this.mapMarker = new google.maps.Marker({
            position: centerPos, map: this.googleMap, draggable: true, title: "Drag to exact location"
        });
    ...
    submitFinalOrder() {
        ...
        const orderData = { delivery_method: delivery, note: note || 'None', cart_items: [], total_price: 0, customer_info: {} };
        ...
        try {
            if(this.tg && this.tg.sendData) { this.tg.sendData(JSON.stringify(orderData)); } 
            else if(this.tg && this.tg.openTelegramLink) { this.tg.openTelegramLink(`https://t.me/${this.supportUsername}?text=${encodeURIComponent(message)}`); } 
            else { window.open(`https://t.me/${this.supportUsername}?text=${encodeURIComponent(message)}`, '_blank'); }
        } catch(e) { window.open(`https://t.me/${this.supportUsername}?text=${encodeURIComponent(message)}`, '_blank'); }
